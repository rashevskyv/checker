    function myFunction() {
        ...
        if (x.validity.valid && x.value.length > 8) {
            ...
            var failed='<div class="failed">Уязвимость устранена<br>Вы не сможете прошить приставку!<br><br>Однако, если версия системного ПО на консоли 4.1.0, то её можно прошить с помощью <a class="checker_a" href="http://switch.customfw.xyz/caffeine">Caffeine</div>';
            var maybe='<div class="maybe">Уязвимость возможно устранена<br>Если вы собираетесь купить эту консоль, подумайте ещё раз, если же уже купили, просто попробуйте</div><p class="checker_p">Единственный верный способ, который поможет понять прошивается ваша приставка или нет - <a class="checker_a" href="http://switch.customfw.xyz/fusee-gelee">попробовать</a>. Если приставка прошиваемая, она запустит переданный ей пейлоад. Если нет, экран останется чёрным, хотя в окне tegrarcmGUI и будет показано, что пейлоад проброшен успешно: <br><br>Значение <strong>0x0000</strong> в логе программы означает, что вашу приставку нельзя прошить с помощью <a class="checker_a" href="http://switch.customfw.xyz/fusee-gelee">Fusée Gelée</a><br><br><img class="checker_img" src="http://check.customfw.xyz/tegra.png"></p>';
